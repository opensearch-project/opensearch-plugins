...
`opensearch.pluginzip` is designed to facilitate the OpenSearch plugin zips to be available in maven repo and then can be fetched for dependency using maven coordinates. This plugin identifies the generated OpenSearch plugin zip file distribution, the ouput of `bundlePlugin` task and publishes to local maven repo with standard maven coordinates.
