...
2. Additional work is needed to get coverage report by JaCoCo for the test classes which use [PowerMock](https://github.com/powermock/powermock), because those classes must be instrumented before the test executes. Please see the [wiki]((https://github.com/powermock/powermock/wiki/Code-coverage-with-JaCoCo)) from PowerMock for detail, and it gives an example when using JaCoCo Maven plugin. But for JaCoCo Gradle pluign, there is no native support, see the [issue](https://github.com/gradle/gradle/issues/2429) in `gralde` repository for detail.
